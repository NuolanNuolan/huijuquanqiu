...
- (void)start:(UIButton  *)button{
    [AppDelegate shareAppDelegate].tabBarViewController = [[MainTabBarController alloc]init];
...
- (UIScrollView *)rootScrollView{
    if (!_rootScrollView) {
        ...
        for (int i=0; i<imageViewCount; i++) {
            ...
            NSString *imagename=[NSString  stringWithFormat:@"destroy%d", i];
...
//        [_startButton  addTarget:self action:@selector(start:) forControlEvents: UIControlEventTouchUpInside];
        ...
        _startButton.frame = CGRectMake(screen_Width/2-62.5, screen_Height*946/1323, 125, 40);
        [_startButton setBackgroundImage:[UIImage imageNamed:@"destroy22"] forState:UIControlStateNormal];
        [_startButton setBackgroundColor:[UIColor clearColor]];
