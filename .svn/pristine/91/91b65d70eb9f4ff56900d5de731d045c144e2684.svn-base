...
- (void)setupViews
{
    ...
    UIBarButtonItem *titleBarItem = [[UIBarButtonItem alloc] initWithTitle:self.placeholder style:UIBarButtonItemStyleBordered target:nil action:nil];
    titleBarItem.enabled = NO;
    ...
    [toolBar setItems:@[cancelBarItem, fixBarItem, titleBarItem, fixBarItem, confirmBarItem] animated:YES];
...
- (void)setPlaceholder:(NSString *)placeholder
{
    ...
    UIBarButtonItem *titleBarItem = toolbar.items[2];
    titleBarItem.title = placeholder;
}
...
- (void)setSelectionArray:(NSArray<NSString *> *)selectionArray
{
    _selectionArray = selectionArray;
        ...
        // 默认选中第一项
//    self.selectedRow = self.defaultSelectedRow;
//    [self refreshContent];
    
//    // 刷新picker
//    UIPickerView *inputView = (UIPickerView *)self.inputView;
//    [inputView reloadAllComponents];
//    [inputView selectRow:self.defaultSelectedRow inComponent:0 animated:NO];
}
