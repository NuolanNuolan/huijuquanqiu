...
- (void)layoutSubviews
{
    ...
    if (!_hasInitInset) {
        ...
        if (_state == MJRefreshStateWillRefresh) {
    ...
    if (_scrollView.isDragging) {
        ...
        if (_state == MJRefreshStatePulling && offsetY <= validOffsetY) {
            ...
            if ([_delegate respondsToSelector:@selector(refreshView:stateChange:)]) {
                [_delegate refreshView:self stateChange:MJRefreshStateIdle];
            }
            
            // 回调
            if (_refreshStateChangeBlock) {
                _refreshStateChangeBlock(self, MJRefreshStateIdle);
            }
        } else if (_state == MJRefreshStateIdle && offsetY > validOffsetY) {
            // 转为即将刷新状态
...
- (void)beginRefreshing
{
    if (self.window) {
        ...
        _state = MJRefreshStateWillRefresh;
    }
}
#pragma mark 结束刷新
- (void)endRefreshing
